...
def speed_change(input_audio: np.ndarray, speed: float, sr: int):
    # å°† NumPy æ•°ç»„è½¬æ¢ä¸ºåŽŸå§‹ PCM æµ
...
class TTS_Config:
    ...
    # "zh",#æŒ‰ä¸­è‹±æ··åˆè¯†åˆ«####ä¸å˜
    # "ja",#æŒ‰æ—¥è‹±æ··åˆè¯†åˆ«####ä¸å˜
    # "yue",#æŒ‰ç²¤è‹±æ··åˆè¯†åˆ«####ä¸å˜
    # "ko",#æŒ‰éŸ©è‹±æ··åˆè¯†åˆ«####ä¸å˜
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if if_lora_v3 == True and os.path.exists(path_sovits) == False:
            info = path_sovits + i18n("SoVITS %s åº•æ¨¡ç¼ºå¤±ï¼Œæ— æ³•åŠ è½½ç›¸åº” LoRA æƒé‡" % model_version)
    ...
    def init_sr_model(self):
        ...
        try:
            ...
            print(i18n("ä½ æ²¡æœ‰ä¸‹è½½è¶…åˆ†æ¨¡åž‹çš„å‚æ•°ï¼Œå› æ­¤ä¸è¿›è¡Œè¶…åˆ†ã€‚å¦‚æƒ³è¶…åˆ†è¯·å…ˆå‚ç…§æ•™ç¨‹æŠŠæ–‡ä»¶ä¸‹è½½å¥½"))
    ...
    def to_batch(
        ...
        for batch_idx, index_list in enumerate(batch_index_list):
            ...
            #### ç›´æŽ¥å¯¹phoneså’Œbert_featuresè¿›è¡Œpadã€‚ï¼ˆpaddingç­–ç•¥ä¼šå½±å“T2Sæ¨¡åž‹ç”Ÿæˆçš„ç»“æžœï¼Œä½†ä¸ç›´æŽ¥å½±å“å¤è¯»æ¦‚çŽ‡ã€‚å½±å“å¤è¯»æ¦‚çŽ‡çš„ä¸»è¦å› ç´ æ˜¯maskçš„ç­–ç•¥ï¼‰
            ...
            # #### å…ˆå¯¹phonesè¿›è¡Œembeddingã€å¯¹bert_featuresè¿›è¡Œprojectï¼Œå†padåˆ°ç›¸åŒé•¿åº¦ï¼Œï¼ˆpaddingç­–ç•¥ä¼šå½±å“T2Sæ¨¡åž‹ç”Ÿæˆçš„ç»“æžœï¼Œä½†ä¸ç›´æŽ¥å½±å“å¤è¯»æ¦‚çŽ‡ã€‚å½±å“å¤è¯»æ¦‚çŽ‡çš„ä¸»è¦å› ç´ æ˜¯maskçš„ç­–ç•¥ï¼‰
    ...
    def run(self, inputs: dict):
        ...
        chunk_split_thershold = 0.0 # è¯¥å€¼ä»£è¡¨è¯­ä¹‰tokenä¸Žmute tokençš„ä½™å¼¦ç›¸ä¼¼åº¦é˜ˆå€¼ï¼Œè‹¥å¤§äºŽè¯¥é˜ˆå€¼ï¼Œåˆ™è§†ä¸ºå¯åˆ‡åˆ†ç‚¹ã€‚

        if parallel_infer and not streaming_mode:
            print(i18n("å¹¶è¡ŒæŽ¨ç†æ¨¡å¼å·²å¼€å¯"))
            ...
            print(i18n("ä¸æ”¯æŒåŒæ—¶å¼€å¯å¹¶è¡ŒæŽ¨ç†å’Œæµå¼æŽ¨ç†æ¨¡å¼ï¼Œå·²è‡ªåŠ¨å…³é—­å¹¶è¡ŒæŽ¨ç†æ¨¡å¼"))
            parallel_infer = False
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_naive
        else:
            print(i18n("æœ´ç´ æŽ¨ç†æ¨¡å¼å·²å¼€å¯"))
            self.t2s_model.model.infer_panel = self.t2s_model.model.infer_panel_naive_batched

        if return_fragment and streaming_mode:
            print(i18n("æµå¼æŽ¨ç†æ¨¡å¼ä¸æ”¯æŒåˆ†æ®µè¿”å›žï¼Œå·²è‡ªåŠ¨å…³é—­åˆ†æ®µè¿”å›ž"))
            return_fragment = False

        if (return_fragment or streaming_mode) and split_bucket:
            print(i18n("åˆ†æ®µè¿”å›žæ¨¡å¼/æµå¼æŽ¨ç†æ¨¡å¼ä¸æ”¯æŒåˆ†æ¡¶å¤„ç†ï¼Œå·²è‡ªåŠ¨å…³é—­åˆ†æ¡¶å¤„ç†"))
            split_bucket = False
        ...
        if split_bucket and speed_factor == 1.0 and not (self.configs.use_vocoder and parallel_infer):
            ...
            print(i18n("è¯­é€Ÿè°ƒèŠ‚ä¸æ”¯æŒåˆ†æ¡¶å¤„ç†ï¼Œå·²è‡ªåŠ¨å…³é—­åˆ†æ¡¶å¤„ç†"))
            split_bucket = False
        elif self.configs.use_vocoder and parallel_infer:
            print(i18n("å½“å¼€å¯å¹¶è¡ŒæŽ¨ç†æ¨¡å¼æ—¶ï¼ŒSoVits V3/4æ¨¡åž‹ä¸æ”¯æŒåˆ†æ¡¶å¤„ç†ï¼Œå·²è‡ªåŠ¨å…³é—­åˆ†æ¡¶å¤„ç†"))
            split_bucket = False
        else:
        ...
        if not no_prompt_text:
            ...
            if prompt_text[-1] not in splits:
                prompt_text += "ã€‚" if prompt_lang != "en" else "."
            print(i18n("å®žé™…è¾“å…¥çš„å‚è€ƒæ–‡æœ¬:"), prompt_text)
        ...
        try:
            ...
            for item in data:
                ...
                print(i18n("å‰ç«¯å¤„ç†åŽçš„æ–‡æœ¬(æ¯å¥):"), norm_text)
                if no_prompt_text:
                    prompt = None
                else:
                    prompt = (
                ...
                if not streaming_mode:
                    ...
                    #     ))
                    print(f"############ {i18n('åˆæˆéŸ³é¢‘')} ############")
                    if not self.configs.use_vocoder:
                        if speed_factor == 1.0:
                            print(f"{i18n('å¹¶è¡Œåˆæˆä¸­')}...")
                            # ## vitså¹¶è¡ŒæŽ¨ç† method 2
                            pred_semantic_list = [item[-idx:] for item, idx in zip(pred_semantic_list, idx_list)]
                            ...
                            for i, idx in enumerate(tqdm(idx_list)):
                                ...
                                batch_audio_fragment.append(audio_fragment)  ###è¯•è¯•é‡å»ºä¸å¸¦ä¸Špromptéƒ¨åˆ†
                    else:
                        if parallel_infer:
                            print(f"{i18n('å¹¶è¡Œåˆæˆä¸­')}...")
                    ...
                    for semantic_tokens, is_final in semantic_token_generator:
                        ...
                        if not self.configs.use_vocoder:
                            ...
                            raise RuntimeError(i18n("SoVits V3/4æ¨¡åž‹ä¸æ”¯æŒæµå¼æŽ¨ç†æ¨¡å¼"))
            ...
            traceback.print_exc()
            # å¿…é¡»è¿”å›žä¸€ä¸ªç©ºéŸ³é¢‘, å¦åˆ™ä¼šå¯¼è‡´æ˜¾å­˜ä¸é‡Šæ”¾ã€‚
            yield 16000, np.zeros(int(16000), dtype=np.int16)
            # é‡ç½®æ¨¡åž‹, å¦åˆ™ä¼šå¯¼è‡´æ˜¾å­˜é‡Šæ”¾ä¸å®Œå…¨ã€‚
            del self.t2s_model
            del self.vits_model
            self.t2s_model = None
    ...
    def empty_cache(self):
        try:
            gc.collect()  # è§¦å‘gcçš„åžƒåœ¾å›žæ”¶ã€‚é¿å…å†…å­˜ä¸€ç›´å¢žé•¿ã€‚
    ...
    def audio_postprocess(
        ...
        if super_sampling:
            ...
            t2 = time.perf_counter()
            print(f"è¶…é‡‡æ ·ç”¨æ—¶ï¼š{t2 - t1:.3f}s")
        else:
